...
prepareDmg()
{
	...
	cp -f ../../README "/Volumes/$APP_BASENAME"
	
	# Create Applications symlink for install_name_tool
	ln -s "/Applications" "/Volumes/$APP_BASENAME/Applications"
	
	# cleanup
	echo "Cleaning up..."
	curPath=$PWD
	target="/Volumes/$APP_BASENAME"
	cd "$target"
	rm -rf ./PlugIns
